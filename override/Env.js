...
// const $env = new Env('xxx')

/**
 * 通用方法 适配 Quantumult X Loon Surge Shadowrocket Stash 
 */
function Env(name, opts) {
    ...
    return new (class {
       ...
       * 获取json key对应的值(对象)
       */
      get_json(key, defaultValue) {
        let json = defaultValue
        const val = this.get_data(key)
        if (val) {
          try {
            json = JSON.parse(this.get_data(key))
          } catch {}
        }
        return json
      }
      ...
      set_json(val, key) {
        try {
          return this.set_data(JSON.stringify(val), key)
        } catch {
          return false
        }
      }
      ...
      runScript(script, runOpts) {
        return new Promise((resolve) => {
          let httpapi = this.get_data('@chavy_boxjs_userCfgs.httpapi')
          httpapi = httpapi ? httpapi.replace(/\n/g, '').trim() : httpapi
          let httpapi_timeout = this.get_data('@chavy_boxjs_userCfgs.httpapi_timeout')
      ...
      get_data(key) {
        ...
        if (/^@/.test(key)) {
          ...
          const objval = objkey ? this.get_val(objkey) : ''
          if (objval) {
            try {
      ...
      set_data(val, key) {
        ...
        if (/^@/.test(key)) {
          const [, objkey, paths] = /^@(.*?)\.(.*?)$/.exec(key)
          const objdat = this.get_val(objkey)
          const objval = objkey ? (objdat === 'null' ? null : objdat || '{}') : '{}'
          try {
            const objedval = JSON.parse(objval)
            this.lodash_set(objedval, paths, val)
            issuc = this.set_val(JSON.stringify(objedval), objkey)
          } catch (e) {
            const objedval = {}
            this.lodash_set(objedval, paths, val)
            issuc = this.set_val(JSON.stringify(objedval), objkey)
